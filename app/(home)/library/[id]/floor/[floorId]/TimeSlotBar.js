...
// Group adjacent available slots into continuous blocks
function buildBlocks(hours) {
  ...
  for (const slot of hours) {
    ...
    if (available) {
      const slotDuration = (slot.granularity || 30);
...
// Find which block a time belongs to
function findBlock(blocks, time) {
...
export default function TimeSlotBar({ hours, onSelect }) {
  ...
  lastEnd.setMinutes(lastEnd.getMinutes() + (lastSlot.granularity || 30));
  const timelineEnd = timeToMinutes(
    `${lastEnd.getHours().toString().padStart(2, '0')}:${lastEnd.getMinutes().toString().padStart(2, '0')}`
  );
  ...
  const handleClick = (time) => {
    if (!selStart || (selStart && selEnd)) {
      ...
      if (clickedMin === startMin) {
        // Same slot clicked — book just this one
        setSelEnd(time);
      } else {
        // Ensure start < end
        if (clickedMin < startMin) {
  ...
  // When selection is complete, notify parent
  const selectionComplete = selStart && selEnd;
  if (selectionComplete) {
    const selDuration = allSlots.find(s => s.hour === selEnd)?.granularity || 30;
  ...
  const selectedEndMin = selEnd ? timeToMinutes(selEnd) + (allSlots.find(s => s.hour === selEnd)?.granularity || 30) : null;
    ...
    <div className="w-full">
      ...
      <div className="relative h-5 mb-1">
        ...
        {selectionComplete && (
          <>
            <span
              className="absolute text-xs font-semibold text-blue-700"
              style={{
                left: `${((selectedStartMin - timelineStart) / totalRange) * 100}%`,
            ...
            <span
              className="absolute text-xs font-semibold text-blue-700"
              style={{
                left: `${((selectedEndMin - timelineStart) / totalRange) * 100}%`,
                transform: 'translateX(-50%)',
              }}
            >
              {selEnd}
      ...
      <div className="relative w-full h-10 rounded-lg overflow-hidden bg-gray-200 flex">
        {blocks.map((block, i) => {
          ...
          const inSelection = selectionComplete &&
            selectedStartMin < blockEnd &&
            selectedEndMin > blockStart;

          // Render individual slots within the block for click targets
          return (
            <div
              key={i}
            ...
            >
              {block.slots.map((slot, j) => {
                ...
                const slotInSel = selectionComplete &&
                  selectedStartMin < slotEnd &&
                  selectedEndMin > slotStart;

                // Is this the start or end of selection?
                ...
                return (
                  <button
                    ...
                    style={{
                      backgroundColor: slotInSel
                        ? (isSelStart || isSelEnd ? '#1d4ed8' : '#3b82f6')
                        : '#22c55e',
                    }}
        ...
        <div className="mt-3 flex items-center justify-between bg-blue-50 border border-blue-200 rounded-lg px-4 py-2.5">
          <div>
            ...
            <span className="text-sm text-blue-600 ml-2">
              ({formatDuration(selectedStartMin, selectedEndMin)})
            </span>
          </div>
          <button
      ...
      {/* Hint */}
      {!selectionComplete && selStart && (
        <p className="mt-2 text-sm text-gray-500 text-center">
          Click another slot to select a range
        </p>
      )}
      {!selStart && (
        <p className="mt-2 text-sm text-gray-500 text-center">
          Click a slot to start selection
        </p>
      )}
    </div>
  );
